...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        self.convolution1 = nn.Conv2d(in_channels=channels,out_channels=num_filters,kernel_size=kernel_size,stride=stride_conv,padding=padding)
        self.convolution1.weight.data.mul_(weight_scale)
        self.relu1 = nn.ReLU()
        self.maxpool1 = nn.MaxPool2d(kernel_size=pool,stride=stride_pool)
        H_new = int((height - kernel_size + 2*padding +1)/pool)
        W_new = int((height - kernel_size + 2*padding +1)/pool)
        self.fc1 = nn.Linear(H_new*W_new*num_filters,hidden_dim)
